 ...
 * @since v1.0
 */
if ( ! function_exists( 'apagao_dos_apps_setup_theme' ) ) :
	function apagao_dos_apps_setup_theme() {
		// Make theme available for translation: Translations can be filed in the /languages/ directory.
		load_theme_textdomain( 'apagao_dos_apps', get_template_directory() . '/languages' );
	...
	add_action( 'after_setup_theme', 'apagao_dos_apps_setup_theme' );
 ...
 * @since v1.0
 */
if ( ! function_exists( 'apagao_dos_apps_add_user_fields' ) ) :
	function apagao_dos_apps_add_user_fields( $fields ) {
		// Add new fields.
	...
	add_filter( 'user_contactmethods', 'apagao_dos_apps_add_user_fields' ); // get_user_meta( $user->ID, 'facebook_profile', true );
endif;
 ...
 * @since v1.0
 */
function apagao_dos_apps_filter_media_comment_status( $open, $post_id = null ) {
...
add_filter( 'comments_open', 'apagao_dos_apps_filter_media_comment_status', 10, 2 );
...
function apagao_dos_apps_custom_edit_comment_link( $output ) {
	return str_replace( 'class="comment-edit-link"', 'class="comment-edit-link badge badge-secondary"', $output );
}
add_filter( 'edit_comment_link', 'apagao_dos_apps_custom_edit_comment_link' );
 ...
 * @since v1.0
 */
function apagao_dos_apps_oembed_filter( $html ) {
	return '<div class="ratio ratio-16x9">' . $html . '</div>';
}
add_filter( 'embed_oembed_html', 'apagao_dos_apps_oembed_filter', 10, 4 );


if ( ! function_exists( 'apagao_dos_apps_content_nav' ) ) :
	/**
	 * Display a navigation to next/previous pages when applicable.
	 *
	 * @since v1.0
	 */
	function apagao_dos_apps_content_nav( $nav_id ) {
		global $wp_query;
 ...
 * @since v1.0
 */
function apagao_dos_apps_widgets_init() {
	// Area 1.
	register_sidebar(
		array(
...
add_action( 'widgets_init', 'apagao_dos_apps_widgets_init' );


if ( ! function_exists( 'apagao_dos_apps_article_posted_on' ) ) :
	/**
	...
	function apagao_dos_apps_article_posted_on() {
		printf(
			...
			sprintf( esc_attr__( 'View all posts by %s', 'apagao_dos_apps' ), get_the_author() ),
			get_the_author()
		);
	}
endif;
 ...
 * @since v1.0
 */
function apagao_dos_apps_password_form() {
	global $post;
...
add_filter( 'the_password_form', 'apagao_dos_apps_password_form' );


if ( ! function_exists( 'apagao_dos_apps_comment' ) ) :
	/**
	 * Style Reply link.
	 *
	 * @since v1.0
	 ...
	 * add function to comments.php ... wp_list_comments( array( 'callback' => 'apagao_dos_apps_comment' ) );
	 *
	 * @since v1.0
	 */
	function apagao_dos_apps_comment( $comment, $args, $depth ) {
	...
	?>
		<li class="post pingback">
			<p><?php esc_html_e( 'Pingback:', 'apagao_dos_apps' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'apagao_dos_apps' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
				break;
			default:
	?>
				...
				<footer class="comment-meta">
					<div class="comment-author vcard">
						<?php
							...
							edit_comment_link( __( 'Edit', 'apagao_dos_apps' ), '<span class="edit-link">', '</span>' );
						?>
					</div><!-- .comment-author .vcard -->

					<?php if ( '0' === $comment->comment_approved ) : ?>
						<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'apagao_dos_apps' ); ?></em>
						<br />
					<?php endif; ?>
				</footer>
	 ...
	 */
	function apagao_dos_apps_custom_commentform( $args = array(), $post_id = null ) {
		if ( null === $post_id ) {
			$post_id = get_the_ID();
		}
		...
		$fields   = array(
			...
			'email'   => '<div class="form-floating mb-3">
							...
							<label for="email">' . esc_html__( 'Email', 'apagao_dos_apps' ) . ( $req ? '*' : '' ) . '</label>
						</div>',
			'url'     => '',
	...
	add_filter( 'comment_form_defaults', 'apagao_dos_apps_custom_commentform' );
 ...
 * @since v1.0
 */
function apagao_dos_apps_scripts_loader() {
...
add_action( 'wp_enqueue_scripts', 'apagao_dos_apps_scripts_loader' );
